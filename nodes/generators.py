...
# Preset text constants for TestStringGenerator
_STRING_PRESETS = {
    "empty": "",
    "short_prompt": "a cat sitting on a windowsill",
    "detailed_prompt": (
        "a photorealistic portrait of an elderly woman with silver hair, "
        "soft natural lighting, shallow depth of field, shot on Canon EOS R5, "
        "8k uhd, highly detailed, masterpiece, best quality"
    ),
    "negative_prompt": (
        "blurry, low quality, worst quality, jpeg artifacts, watermark, "
        "text, signature, deformed, ugly, duplicate, morbid, mutilated, "
        "out of frame, extra limbs, bad anatomy, bad proportions"
    ),
    "special_characters": "test (weight:1.5) [optional] {variable} \"quoted\" 'single' line1\nline2\ttab pipe|sep colon:value",
    "csv_list": "red, green, blue, yellow, orange, purple, white, black",
    "template": "a {subject} in the style of {artist}, {quality}",
    "multiline": "First line of the prompt.\nSecond line with more detail.\nThird line with quality tags: masterpiece, best quality.",
}


class TestStringGenerator(io.ComfyNode):
    """Generates test strings with known content for assertion-based testing.

    Preset options:
      empty              — Empty string. Tests edge cases and empty input handling.
      short_prompt       — Simple one-line prompt for basic string operations.
      detailed_prompt    — Long multi-clause prompt with quality tags and camera details.
      negative_prompt    — Typical negative prompt with common exclusion terms.
      special_characters — Stress-test with parentheses, brackets, quotes, newlines, tabs, pipes, colons.
      csv_list           — Comma-separated color names for split/list nodes.
      template           — Template string with {variable} placeholders.
      multiline          — Multi-line text with newline characters.
      custom             — User-provided text via the custom_text input.
    """

    @classmethod
    def define_schema(cls) -> io.Schema:
        return io.Schema(
            node_id="TestStringGenerator",
            display_name="Test String Generator",
            category="testing",
            description="Generate test strings: preset prompts, special characters, templates, or custom text",
            inputs=[
                io.Combo.Input(
                    "preset",
                    options=[
                        "empty", "short_prompt", "detailed_prompt", "negative_prompt",
                        "special_characters", "csv_list", "template", "multiline", "custom",
                    ],
    ...
    def execute(
        ...
        """Generate test string based on preset or custom input."""

        if preset == "custom":
            text = custom_text
        else:
            text = _STRING_PRESETS.get(preset, "")
